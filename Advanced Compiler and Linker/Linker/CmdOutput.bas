...
Private Declare Function OemToCharBuff Lib "user32" _
      Alias "OemToCharBuffA" ( _
      lpszSrc As Any, _
...
' blnOEMConvert:   [in,opt] True (default) to convert DOS characters to Windows, False to skip
'   conversion
'
...
Public Function GetCommandOutput( sCommandLine As String, _
  ...
  Const BUFSIZE = 1024      ' pipe buffer size
   ...
   Call DuplicateHandle(hCurProcess, hPipeRead, hCurProcess, hPipeRead, 0&, 0&, _
      DUPLICATE_SAME_ACCESS Or DUPLICATE_CLOSE_SOURCE)

   ' If both STDOUT and STDERR should be redirected, get an extra handle.

   If fTwoHandles Then
      Call DuplicateHandle(hCurProcess, hPipeWrite1, hCurProcess, hPipeWrite2, 0&, 1&, _
   ...
   If CreateProcess(vbNullString, sCommandLine, ByVal 0&, ByVal 0&, 1, 0&, ByVal 0&, vbNullString, _
      si, pi) Then
